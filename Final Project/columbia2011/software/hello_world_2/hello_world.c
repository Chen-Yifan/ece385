...
unsigned char transmit_buffer[] = {
  ...
  0xc0,0xa8,0x01,0x02, // source IP address
...
static void ethernet_interrupt_handler() {
  ...
  //int i;
  int x,y;
  printf("package %d action %c ", test++,receive_buffer[42]);
...
void move(int x,int y){
  ...
  //        printf("number %d x  %d y %d\n",package,x,y);
     //     package++;
          ...
          UDP_PACKET_PAYLOAD[curMsgChar++] = 2;
 ...
 while(!IORD_8DIRECT(PS2_BASE,0)||(g.current_time%schedule!=turn)){
  ...
  if(g.step == 1){
   switch(code){
     ...
     case 117: //up
         p2.pos1X = (p2.posX - p2.posX%32)/32;
         tempxleft = (p2.posX-5)/32;
         tempxright = (p1.posX+5)/32;
         p2.pos1Y = (p2.posY - p2.posY%32)/32; 
         if(control_array[p2.pos1Y-1][p2.pos1X]==EMPTY
          ||control_array[p2.pos1Y-1][p2.pos1X]==FIRE 
          ||control_array[p2.pos1Y-1][p2.pos1X]==POWERUP
          ||control_array[p2.pos1Y-1][p2.pos1X]==BOMBUP
          ||control_array[p2.pos1Y-1][p2.pos1X]==SPEEDUP)          
         p2.posY -= p2.speed;
         else{
         p2.posY -= p2.speed;   
         if(p2.posY <= p2.pos1Y*32+16)
               p2.posY = p2.pos1Y*32+15;                        
         }
         move(p2.posX,p2.posY);
         usleep(10000);
     break;
     case 114: //down   
         p2.pos1X = (p2.posX - p2.posX%32)/32;
     ...
     case 107: //left
         ...
         p2.pos1Y = (p2.posY - p2.posY%32)/32;
         tempy  = (p2.posY +14)/32;   
         if((control_array[p2.pos1Y][p2.pos1X-1]==EMPTY&&
             control_array[tempy][p2.pos1X-1]==EMPTY)
          ||control_array[p2.pos1Y-1][p2.pos1X]==FIRE    
          ||control_array[p2.pos1Y][p2.pos1X-1]==POWERUP
          ||control_array[p2.pos1Y][p2.pos1X-1]==BOMBUP
          ||control_array[p2.pos1Y][p2.pos1X-1]==SPEEDUP)  
          p2.posX -= p2.speed;
         else{
         p2.posX -= p2.speed;
         if(p2.posX <= p2.pos1X*32+16)
               p2.posX = p2.pos1X*32+15;                  
         }
         move(p2.posX,p2.posY);
         usleep(10000);
     break;
     ...
     case 41:
         p2.pos1X = (p2.posX - p2.posX%32)/32;
         p2.pos1Y = (p2.posY - p2.posY%32)/32;
         if(control_array[p2.pos1Y][p2.pos1X] == EMPTY && checkbomb(p2.bomb)){
             
             insert_bomb(p2.pos1X,p2.pos1Y, p2.power,player2);             
             set_tile(p2.pos1X,p2.pos1Y,BOMB);
             put_bomb(p2.pos1X, p2.pos1Y,p2.power);
             usleep(10000);              
         } 
    ...
    if(control_array[p2.pos1Y][p2.pos1X]==BOMBUP){
       p2.bomb++;
       set_tile(p2.pos1X,p2.pos1Y,EMPTY);  
    }
    if(control_array[p2.pos1Y][p2.pos1X]==SPEEDUP){
       if(p2.speed < 10)
       p2.speed++;
       set_tile(p2.pos1X,p2.pos1Y,EMPTY);  
    }
